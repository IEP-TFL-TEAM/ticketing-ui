<script>
	...
	const pendingRequsts = countByStatus(data.requests, 'PENDING');
	const closedRequests = countByStatus(data.requests, 'CLOSED');

	const pieData = {
		labels: ['Closed', 'Pending'],
		datasets: [
			{
				data: [closedRequests, pendingRequsts],
	...
	const fullCompletionPercentage =
		data.requests.length > 0 ? (closedRequests / data.requests.length) * 100 : 0;
</script>
